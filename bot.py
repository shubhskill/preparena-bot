...
MAIN_KEYBOARD = InlineKeyboardMarkup([
    ...
    [InlineKeyboardButton("🚪 Logout", callback_data="st|logout")],
])

TESTS_KEYBOARD = InlineKeyboardMarkup([
    [InlineKeyboardButton("🏠 Main Menu", callback_data="st|home")],
    [InlineKeyboardButton("🚪 Logout", callback_data="st|logout")],
])
...
async def available_tests(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not tests:
        await message.reply_text("📝 <b>Available Tests</b>\n\nNo tests are available right now.", parse_mode="HTML", reply_markup=TESTS_KEYBOARD)
        return
    ...
    keyboard.append([InlineKeyboardButton("🚪 Logout", callback_data="st|logout")])
...
async def open_test(query, test_id: str) -> None:
    try:
        ...
        if test.get("instructions"):
            text += f"\n📋 <b>Instructions</b>\n{test['instructions']}\n"
        await query.edit_message_text(text, parse_mode="HTML", reply_markup=InlineKeyboardMarkup([[action], [InlineKeyboardButton("⬅️ Back", callback_data="st|tests")], [InlineKeyboardButton("🚪 Logout", callback_data="st|logout")]]))
    except Exception:
...
def answer_keyboard(question_id: str, question_type: str, selected: list[str]) -> InlineKeyboardMarkup:
    if question_type == "NUMERICAL":
        ...
        for i in range(0, len(opts), 2):
            ...
            rows.append(row)
    rows.append([InlineKeyboardButton("🚪 Logout", callback_data="st|logout")])
...
async def _callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if action == "logout":
        ...
        awaiting_numerical.pop(update.effective_user.id, None)
        await query.edit_message_text(
            "🚪 <b>Logged out</b>\n\nUse /start whenever you want to open PrepArena again.",
            parse_mode="HTML",
        )
    ...
    if action == "home":
        await query.edit_message_text(main_text(), parse_mode="HTML", reply_markup=MAIN_KEYBOARD)
        await query.answer("Main menu")
        return

    if action == "tests":
        # Render the test list in the same message so the old menu is replaced.
        if not private(update):
            await query.answer("Please open the bot privately to view tests.", show_alert=True)
            return
        try:
            response = (
                supabase.table("tests")
            ...
            keyboard = []
            for test in tests:
                icon = "🟢" if test.get("status") == "LIVE" else "🗓"
                keyboard.append([InlineKeyboardButton(
                    f"{icon} {test.get('name') or 'Unnamed Test'}",
                    callback_data=f"st|open|{test['id']}",
                )])
            keyboard.append([InlineKeyboardButton("🏠 Main Menu", callback_data="st|home")])
            keyboard.append([InlineKeyboardButton("🚪 Logout", callback_data="st|logout")])
            await query.edit_message_text(
            ...
            logger.exception("Failed to load tests from callback")
            await query.answer("Could not load tests right now.", show_alert=True)
        return
    ...
    if action in ("ans", "num") and len(parts) >= 3:
        ...
        if action == "num":
            ...
            await query.message.reply_text(
                "🔢 Send your numerical answer as a number.",
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🚪 Logout", callback_data="st|logout")]]),
            )
            await query.answer("Enter your answer")
            return
...
async def results(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        if not rows:
            await message.reply_text("🏆 <b>My Results</b>\n\nNo results are available yet.", parse_mode="HTML", reply_markup=RESULTS_KEYBOARD)
            return
        ...
        await message.reply_text("\n".join(lines), parse_mode="HTML", reply_markup=RESULTS_KEYBOARD)
    except Exception:
...
async def support(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not message:
        return
    await message.reply_text("💬 <b>Support</b>\n\nSend your support message in your next message. It will be forwarded to the PrepArena support account.", parse_mode="HTML", reply_markup=SUPPORT_KEYBOARD)

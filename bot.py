...
MAIN_KEYBOARD = ReplyKeyboardMarkup(
    [["📝 Available Tests", "🏆 My Results"], ["👤 My Profile", "💬 Support"]],
...
async def available_tests(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        await update.message.reply_text("❌ Could not load tests right now.")
        return

    if not tests:
        await update.message.reply_text("📝 <b>Available Tests</b>\n\nNo tests are available right now.", parse_mode="HTML", reply_markup=MAIN_KEYBOARD)
        return
...
async def results(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        await update.message.reply_text("❌ Could not load your results right now.")
...
async def support(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if not private(update):
        return
    await update.message.reply_text("💬 Send your support message in your next message. It will be forwarded to the PrepArena support account.")
...
async def support_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not SUPPORT_CHAT_ID:
        await update.message.reply_text("⚠️ Support is not configured yet. Please try again later.", reply_markup=MAIN_KEYBOARD)
        return
    try:
        user = update.effective_user
        header = f"💬 PrepArena Student Support\nFrom: {display_name(update)}\nUsername: @{user.username}" if user and user.username else f"💬 PrepArena Student Support\nFrom: {display_name(update)}"
        await context.bot.send_message(chat_id=int(SUPPORT_CHAT_ID), text=header + "\n\n" + update.message.text)
        await update.message.reply_text("✅ Your message has been sent to support.", reply_markup=MAIN_KEYBOARD)
    except Exception:
        logger.exception("Failed to forward support")
        await update.message.reply_text("❌ Could not send your message right now.", reply_markup=MAIN_KEYBOARD)
...
async def menu_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if context.user_data.get("awaiting_support"):
        ...
        return
    if text == "📝 Available Tests":
        await available_tests(update, context)
    elif text == "🏆 My Results":
...
def main() -> None:
    ...
    app.add_handler(CallbackQueryHandler(callbacks, pattern=r"^st\|"))
